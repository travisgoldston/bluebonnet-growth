...
const SOCIAL = `        <div class="footer-social" aria-label="Social media">
...
for (const file of walk(ROOT)) {
  ...
  if (!html.includes("footer-social") && html.includes("footer-tagline")) {
    html = html.replace(
      /(<div class="footer-tagline">[\s\S]*?<\/div>)/,
      `$1\n${SOCIAL}`
    );
    changed = true;
  }

  if (file.includes(`${path.sep}blog${path.sep}`) && html.includes("<body>")) {
    html = html.replace("<body>", '<body class="blog-post">');
    changed = true;
  }
